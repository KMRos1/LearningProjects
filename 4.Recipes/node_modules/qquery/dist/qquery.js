...
const QQuery = class extends Array {
    ...
    html(val = null) {
        return twoWays(this, val, item => item.innerHTML, (item, val) => item.innerHTML = val);
    }
    text(val = null) {
        return twoWays(this, val, item => item.textContent, (item, val) => item.textContent = val);
    }

    val(val = null) {
        return twoWays(this, val, item => item.value, (item, val) => item.value = val);
    }
    width(val = null) {
        return twoWays(this, val, item => item.clientWidth, (item, val) => item.style.width = toPixels(val));
    }
    height(val = null) {
        return twoWays(this, val, item => item.clientHeight, (item, val) => item.style.height = toPixels(val));
    }
    attr(key, val = null) {
        return twoWaysMultiple(this, key, val, (item, key) => item.getAttribute(key), (item, key, val) => item.setAttribute(key, val));
    }
    data(key, val = null) {
        return twoWaysMultiple(this, key, val, (item, key) => item.dataset[key], (item, key, val) => item.dataset[key] = val);
    }
    css(key, val = null) {
        return twoWaysMultiple(this, key, val, (item, key) => item.style.getPropertyValue(key), (item, key, val) => item.style.setProperty(key, val));
    }
    hasClass(key) {
